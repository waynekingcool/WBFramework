...
+ (CGFloat)wrscreenWidth {
    return [UIScreen mainScreen].bounds.size.width;
}
+ (CGFloat)wrscreenHeight {
    ...
    for (UIView *view in self.subviews) {
        ...
        if (_UIBarBackgroundClass != nil) {
            if ([view isKindOfClass:_UIBarBackgroundClass]) {
                view.frame = CGRectMake(0, self.frame.size.height-[WRNavigationBar navBarBottom], [WRNavigationBar wrscreenWidth], [WRNavigationBar navBarBottom]);
            }
        }
    }
}
